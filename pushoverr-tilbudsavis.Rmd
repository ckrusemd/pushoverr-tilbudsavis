...
keywords = c("cheasy skyr","stay strong","tun","cola","hakket oksekød")
...
df_etilbudsavisen = do.call("rbind",lapply(keywords,function (keyword) {
  ...
  distinct() %>%
  filter(name %in% c("Netto","føtex","MENY","Coop365","REMA 1000","Bilka")) %>% 
...
msg = df_etilbudsavisen %>% 
  ...
  pull(text) %>% 
  paste(.,collapse="\n")
msg = glue("You will be provided a table of shopping offers. Do not write any introducing or conclusion text. You must keep under 1024 characters. List the best deals if I want to buy: 1) Skyr, not yoghurt skyr, 2) Tuna, the cheapest per amount, 3) Sodavand, particularly Coca Cola Zero or Faxe Kondi Free, t least 1 liter 4) Yoggi yoghurt, 5) Pizzabund, 6) Philadelphia. List in the format of 1) The Store's name, 2) Bullet-points of the item name, the price. Compile offers from the same store into the same group. \n\n{msg}")
msg
```
