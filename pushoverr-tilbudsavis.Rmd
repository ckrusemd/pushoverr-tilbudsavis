...
df_etilbudsavisen = do.call("rbind",lapply(keywords,function (keyword) {
  ...
  temp_df = result %>% 
    ...
    bind_cols(images_) %>% 
    dplyr::mutate(images=paste0("<img src='",images,"' height='300' width='300'></img>"))
...
```{r}
df_etilbudsavisen %>% filter(keyword=="nyborg") %>% arrange(heading) %>%   show_tilbud(.)
```
...
```{r}
df_etilbudsavisen %>% filter(keyword %in% c("slik")) %>%  arrange(price_per_kg) %>% show_tilbud(.)
```

### Legetøj

```{r}
df_etilbudsavisen %>% filter(keyword %in% c("lego","switch","playstation","ps5")) %>%  arrange(price_per_kg) %>% show_tilbud(.)
```
