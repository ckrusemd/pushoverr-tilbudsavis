...
```{r}
df_etilbudsavisen %>% filter(keyword %in% c("neutral","tabs","tandpasta")) %>%  arrange(price_per_kg) %>% DT::datatable(.,filter="top")
```
...
```{r}
df_etilbudsavisen %>% filter(keyword %in% c("bilka")) %>%  arrange(price_per_kg) %>% DT::datatable(.,filter="top")
```
...
pushoverr_string = df_etilbudsavisen %>% 
  filter(keyword %in% c("skyr","tun","sodavand","oksekød","starbucks"),
